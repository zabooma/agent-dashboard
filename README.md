...
Agents place their own cards by reporting a status. When an agent stops reporting — a session that hit a limit, a stopped process, a handoff nobody picked up — the card it was on stays in the wrong column indefinitely, and no amount of waiting fixes it. Every card's footer therefore carries **Move**: it opens a two-by-two panel of the four lanes, with the lane the card is in marked rather than hidden, so the human can see what a move would change. The panel is part of the card's own flow rather than a floating menu, because `.session-card` clips its overflow and a popover anchored in the footer would be cut off.

A move is a **placement, never a status report**. The card keeps the chip its agents earned — a card moved into Done while its agent still says `working` still says `working` — and gains a **Moved by you** chip which is the only thing on the board that reports a human's decision rather than an agent's claim. Four consequences are deliberate:

- **It stays until the human clears it.** A live session reporting every few seconds must not be able to silently undo a deliberate move, or the control would appear to work and then revert under the pointer. **Return to the agents** in the same panel is the way back, and the card then follows its statuses again.
- **It does not touch `updatedAt`.** A card's time is when an agent last wrote. Stamping a move would print "just now" beside a session that has been silent for a day — the one fact the board exists to report — and would reorder every lane the human tidies.
- **Bookkeeping follows the lane, not the status.** A card moved out of Attention stops being unread and stops counting in *need an answer*, which is what moving it was for; a card the human moves *into* Attention is acknowledged by that same act, so it does not arrive wearing the **New** badge.
- **The advisory stall chip stands down.** It exists to guess at a session that went quiet, and the human has already looked at that card and decided where it belongs.

`laneOverride` on the work session carries the placement — `{ lane, at }`, or `null` for a card its agents own — and travels to every reader, including `list_work_sessions`, so an agent can see that a human placed its card. The page writes it with `PATCH /api/work-sessions/:id` and a body of `{"lane":"<lane>"}` or `{"lane":null}`; the lane is validated against the same list the store exports, and a name the board does not have is a `400` that names the lanes there are. Nothing is redrawn optimistically: the card repaints from the server's answer, so a failed move leaves it where the human found it rather than showing a position it never took.

The lane list exists three times — the columns in `index.html`, the map in `public/board-state.js`, and `LANES` in `lib/dashboard-store.mjs` — because the board is markup, the rules are a module, and the endpoint is server-side. `test/lane-override.test.mjs` holds all three together, in board order. The same file pins the bug that came out of building this against a live board: the panel's targets were first given `data-lane`, and since a card lives *inside* a lane, `document.querySelector('[data-lane="active"]')` then matched a button in an earlier card instead of the column — one card drawn, counters at zero, `OFFLINE`. The targets now carry `data-move-to`, and every lane lookup names the lane element itself.
